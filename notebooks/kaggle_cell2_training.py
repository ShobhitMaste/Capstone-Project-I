...
# --- Dataset Distribution Chart (for Review 3 slides) ---
...
plt.tight_layout()
plt.savefig(str(RESULTS_DIR / "dataset_distribution.png"), dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
print(f"  Total training time: {training_elapsed/60:.1f} minutes")
...
metrics = {
    ...
    "training_time_minutes": round(training_elapsed / 60, 2),
...
plt.tight_layout()
plt.savefig(str(RESULTS_DIR / "per_class_metrics.png"), dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(str(RESULTS_DIR / "confusion_matrix_normalized.png"), dpi=300, bbox_inches='tight')
plt.show()
plt.close()
...
Training Strategy:
  Phase 1 (Epochs 1-{FREEZE_EPOCHS}):   Frozen backbone, train classifier head only
  Phase 2 (Epochs {FREEZE_EPOCHS+1}-{TOTAL_EPOCHS}):  Fine-tune top-3 backbone blocks
...
Federated Learning:
  ...
  FL Rounds:             {FL_ROUNDS}
  Local Epochs/Round:    {FL_LOCAL_EPOCHS}
...
print("  Saved: training_curves.png, confusion_matrix.png, per_class_metrics.png,")
print("         confusion_matrix_normalized.png, model_summary.txt, training_metrics.json")
